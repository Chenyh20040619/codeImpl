...
output_dir = 'result/output.xlsx'
train_semantic = 'train_semantic.txt'
test_semantic = 'test_semantic.txt'
jieba.load_userdict(user_dict_dir)

data = pd.read_excel(train_data_dir)
data.content = data.content.astype(str)

data['segment'] = data['content'].apply(lambda x:jieba.lcut(x))
real = pd.read_excel(test_data_dir)
...
# print(train_data)
# 0                              [感受, 三国, 文化, 地方, 十一, 人山人海]
# 1                        [锦里, 要进, 武候祠, 锦里, 宽窄巷子, 锦里, 武候祠]
# print(train_label)
# 0       1
# 1       1
with open('semantic/train_semantic.txt','w',encoding='utf-8') as f:
    for i in range(len(train_data)):
        str1 = " ".join(train_data[i])+"\t"+"__label__"+str(train_label[i])+'\n'
        f.write(str1)

with open('semantic/test_semantic.txt','w',encoding='utf-8') as f:
...
print('------------------开始训练模型--------------------')
model = fastText.train_supervised(input="semantic/train_semantic.txt",lr=0.1, epoch=100, wordNgrams=3, dim=300)
print('------------------模型训练结束--------------------')
model.save_model(r'result/model.model')
...
print('------------------正在写入测试文件--------------------')
real[['content','pred_score']].to_excel(output_dir,encoding='utf-8')