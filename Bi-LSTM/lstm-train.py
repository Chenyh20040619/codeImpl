...
if __name__ == '__main__':
    ...
    device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device("cpu")  # GPU

    model = BI_lstm(vocab_size, vocab_train, config.n_layers, config.hidden_dim, config.embed, config.output_size,
                    config.dropout)  # 模型实例化
    model.to(device)
    # 训练并保存模型
